...
def lambda_handler(event, context):
    ...
    try:
        ...
        if error_code == "NoSuchPublicAccessBlockConfiguration":
            ...
            log_action("check_state", bucket_name, requested_by, dry_run,
                       "ERROR", f"AccessDenied detail: {str(e)}")
            return {"statusCode": 403, "body": f"Access denied: {str(e)}"}
        else:
            log_action("check_state", bucket_name, requested_by, dry_run,
                       "ERROR", f"Unhandled error_code={error_code}, detail={str(e)}")
            return {"statusCode": 500, "body": f"Unhandled: {error_code} - {str(e)}"}
